...
form.onsubmit =(event) => {
  ...
  // Chama a função que irá adicionar o item na lisa
...
function expenseAdd(newExpense) {
  try {
    // Cria o elemento para adicionar o item (li) na lista(lu).
    const expenseItem = document.createElement("li")
    expenseItem.classList.add("expense")

    // Cria o ícone dea categoria.
...
function updateTotals() {
  try {
    ...
    // Cria a span para adicionar oR$ Formatado
    ...
    console.log(error)
    alert("Não foi possíves atualizar os totais.")
...
expenseList.addEventListener("click", function (event){
  ...
  if(event.target.classList.contains("remove-icon")){
    // Obtém a li pai do elemento clicado(closest é o pai mais próximo)
